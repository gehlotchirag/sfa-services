...
// Skus controller
angular.module('skus').controller('SkusController', ['$scope', '$stateParams', '$location', 'Authentication', 'Skus','$ocLazyLoad','$http',
	function($scope, $stateParams, $location, Authentication, Skus, $ocLazyLoad, $http) {
    ...
    // Controller Logic
    $ocLazyLoad.load('/modules/skus/scripts/xls.js');
    $ocLazyLoad.load('/modules/skus/scripts/jszip-combined.js');
    $ocLazyLoad.load('/modules/skus/scripts/xlsx.js');

    $ocLazyLoad.load('lib/angular-material-data-table/dist/md-data-table.min.css');
    ...
    $scope.normalizeJson = function(obj,filetype)
    {
      ...
      if(filetype==='xlsx')
      {
        ...
        ExcelData = $scope.removeBlankRows(ExcelData,columnHeader.length,columnHeader);
        normalizedJSON.columnHeader = columnHeader;
        normalizedJSON.ExcelData = ExcelData;
        normalizedJSON.ExcelObj = exObj;
    ...
    $scope.removeBlankRows = function(rowData,columnCount,column)
    {
      var rdata;
      var cnt;
      var trimmedRowData =[]
    ...
    $scope.EscapeUmlaute = function(instring)
    {
      ...
      {
        Ismatch = instring.toString().match(/[����������������������������������������������������������������������������������������������]/gi);
    ...
    // Create new Sku
		$scope.create = function() {
			...
			var sku = new Skus ({
				products: $scope.DispData.ExcelObj
			});

			// Redirect after save
			sku.$save(function(response) {
				$location.path('skus/' + response._id);
		...
		$scope.remove = function(sku) {
			if ( sku ) { 
				...
				for (var i in $scope.skus) {
					if ($scope.skus [i] === sku) {
						$scope.skus.splice(i, 1);
					}
				}
			} else {
				$scope.sku.$remove(function() {
					$location.path('skus');
				});
			}
		};
